...
# TODO: Optimize convolution parameters
# TODO: Make class to encapsulate
...
def fully_conn(x_tensor, num_outputs):
    ...
    # ASSUME LAYER IS FLATTED fc1 = flatten(x_tensor) bc its a 2D tensor
    x_tensor_shape = x_tensor.get_shape().as_list()

    weight = tf.Variable(tf.truncated_normal([x_tensor_shape[1], num_outputs], stddev=0.1))
...
def conv_net(x, keep_prob):
    ...
    conv_layer_count = 3
    fc_layer_count = 3
    conv_num_outputs = [32, 64, 500]  # [32, 64, 1024]
    conv_ksize = [[5, 5], [5, 5], [3, 3]]
    conv_strides = [[1, 1], [1, 1], [1, 1]]
    pool_ksize = [[2, 2], [2, 2], [2, 2]]
    pool_strides = [[2, 2], [2, 2], [2, 2]]
    fc_num_outputs = [512, 256, 128]  # [1024, 512, 256]
...
def train_neural_network(session, optimizer, keep_probability, feature_batch, label_batch):
    ...
    # Dont need to run global init - assumed it already happened for each batch
